...
class Aureo_AI_Post_Generator {
     ...
     */
    public static function create_daily_post( $source = "cron", $custom_opts = array() ) {
        $niche       = ! empty( $custom_opts["niche"] ) ? sanitize_text_field( $custom_opts["niche"] ) : get_option( "aureo_ai_niche", "construction, civil engineering , interior" );
        $post_status = ! empty( $custom_opts["post_status"] ) ? sanitize_text_field( $custom_opts["post_status"] ) : get_option( "aureo_ai_post_status", "publish" );
        $default_cat = ! empty( $custom_opts["category"] ) ? sanitize_text_field( $custom_opts["category"] ) : get_option( "aureo_ai_category", "Business & Engineering" );

        // 1. Generate Article from Gemini or Fallback Engine
        $article = Aureo_AI_Gemini_Client::generate_article( $niche, $custom_opts );
        ...
        $post_data = array(
            ...
            "tags_input"    => ! empty( $article["tags"] ) ? (array) $article["tags"] : array( "Engineering", "Architecture", "Aureo AI" ),
        );
        ...
        // 4. Attach Featured Image safely
        self::attach_featured_image( $post_id, $article["title"] );

        // 5. Store Metadata
        $engine = ! empty( $article["source_engine"] ) ? $article["source_engine"] : "Google Gemini";
        update_post_meta( $post_id, "_aureo_ai_generated", "1" );
        update_post_meta( $post_id, "_aureo_ai_model", $engine );
        update_post_meta( $post_id, "_aureo_ai_timestamp", current_time( "mysql" ) );

        self::log( "success", sprintf( 'Created post "%s" via %s (ID: %d)', $article["title"], $engine, $post_id ), $source, $post_id );
    ...
    private static function attach_featured_image( $post_id, $title ) {
        ...
        $temp_file  = download_url( $random_url, 25 );
