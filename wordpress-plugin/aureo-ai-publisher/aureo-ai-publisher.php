 ...
 * Description: Automated AI content engine powered by Google Gemini. Automatically generates and publishes 1–2 high-value, SEO-optimized business and luxury architecture monographs daily with featured images and custom niches.
 * Version: 1.0.0
...
function aureo_ai_activate() {
    ...
    if ( false === get_option( 'aureo_ai_niche' ) ) {
        update_option( 'aureo_ai_niche', 'Monolithic Luxury Architecture, Bespoke Real Estate & Spatial Design' );
    }
    if ( false === get_option( 'aureo_ai_posts_per_day' ) ) {
        update_option( 'aureo_ai_posts_per_day', '1' ); // 1 or 2
    }
    if ( false === get_option( 'aureo_ai_post_status' ) ) {
        update_option( 'aureo_ai_post_status', 'publish' ); // publish or draft
    }
    if ( false === get_option( 'aureo_ai_category' ) ) {
        update_option( 'aureo_ai_category', 'Architecture & Design' );
    }
